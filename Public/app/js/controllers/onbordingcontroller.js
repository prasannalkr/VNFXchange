routerApp.controller('onbordingcontroller', ['$scope', '$sce', '$http', function (scope, sce, http) {
	...
	scope.VnfNameArr = [];
	http.get("http://10.53.172.9:8080/getFilebatNames").then(function (response) {
		scope.VnfNameArr = response.data;
	});	
	scope.diskformat = '';
	scope.container = '';
	scope.protocol = '';
	...
	scope.availablezonetypechange = function (availablezone) {
		...
		console.log("dfccc--" + scope.availablezone)
	}
	...
	scope.onboardfile = function (inname) {
		...
		//Path to write the robot report		
		var report_path = 'C:\\xampp\\htdocs\\VNFXchange\\Public\\Report\\' + addTimeStamp;
		//Path which will be inserted to DB vnfOnboard collection.
		var reportPathHtml = 'http://localhost/VNFXchange/Public/Report/'+ addTimeStamp + '/report.html'
		...
		http({
			...
			params:{Vnfname:VnfNam,vnftypename:vnftype,onboard_image:imgn,onboard_flavour:flvr,servertp:servertyp,Diskformat:diskfor,
			        Container:contype,protoCol:proto,direction:dirc,etherty:ethertype,availZon:availzone,imGnam:imgNa,Grpnam:grpNam,ReportPath:reportPathHtml}
