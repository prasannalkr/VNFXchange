...
app.get('/vnfdashboard', function (req, res) {
	...
	dashCollection.find({}).sort({_id:-1}).toArray(function (err, docs) {
...
app.get('/getFilebatNames', function (req, res) {
	//var namearr = fromDir('C:\\xampp\\htdocs\\VNFXchange', '_onboard.bat');
	var namearr = fromDir('C:\\xampp\\htdocs\\VNFXchange\\Genric_Script', 'Onboarding');
	res.send(namearr);
})
function fromDir(startPath, filter) {
	...
	for (var i = 0; i < files.length; i++) {
		...
		if (stat.isDirectory()) {
			...
			if (!namearr.includes(path.basename(filename, '.bat').split("_onboard")[0])) {
				namearr.push(path.basename(filename, '.bat').split("_onboard")[0])
			}
		}; */
		if (filename.indexOf(filter) >= 0) {
			if (!namearr.includes(path.basename(filename, '.robot').split("_Onboarding")[0])) {
				namearr.push(path.basename(filename, '.robot').split("_Onboarding")[0])
			}
		}
	};
	return namearr
